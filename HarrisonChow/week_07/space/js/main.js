...
function initMap() {
  ...
  window.setInterval(getNewLocationAndDraw(path, map), 5000);
}

function getNewLocationAndDraw(path, map) {
  return function() {
    $.ajax({
      url: baseLocationURL,
      dataType: "JSONP",
      ...
      var oldLocation = path[path.length-1]
